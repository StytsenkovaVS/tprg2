...
def ls(num_seq, a, b):
    ...
    num_seq = list(map(lambda _num : _num / divisor, num_seq))
    num_seq = [np.random.uniform() if num_seq[i] == 0.0 else num_seq[i] for i in range(len(num_seq))]
    num_seq = list(map(lambda _num : a + b * math.log(_num / (1 - _num)), num_seq))
...
def ex(num_seq, a, b):
    ...
    num_seq = list(map(lambda num : -b * math.log(num) + a, list(map(lambda num : num / divisor, num_seq))))
